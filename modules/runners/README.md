...
| <a name="input_enable_on_demand_failover"></a> [enable\_on\_demand\_failover](#input\_enable\_on\_demand\_failover) | Enable on-demand failover. When enabled InsufficientInstanceCapacity errors will be retried with an on-demand instance. When disabled the creation will be retried later. | `bool` | `false` | no |
